...
class GroupActivityBloc extends Bloc<GroupActivityEvent, GroupActivityState> {
  ...
  GroupActivityBloc({
    required CreateGroupActivity createGroupActivity,
  })  : _createGroupActivity = createGroupActivity,
        super(GroupActivityInitial()) {
    on<SubmitGroupActivityEvent>(_onSubmitActivity);
  }

  void _onSubmitActivity(SubmitGroupActivityEvent event, Emitter<GroupActivityState> emit) async {
    emit(GroupActivityLoading());
    final result = await _createGroupActivity(CreateGroupActivityParams(
