...
class MatchmakingRemoteDataSourceImpl implements MatchmakingRemoteDataSource {
  ...
  Future<List<MatchProfileModel>> getPotentialMatches() async {
    try {
      ...
      final double myMaxDistance = (currentUserData['max_distance_preference'] ?? 15.0).toDouble();
      ...
      final List<dynamic> myInterestsData = currentUserData['user_interests'] ?? [];
      final Set<String> myInterests = myInterestsData.map((e) => e['interest_id'].toString()).toSet();
      ...
      for (var row in response) {
        ...
        final double distance = _calculateHaversine(myLat, myLon, otherLat, otherLon);
        ...
        final Set<String> otherInterests = otherInterestsData.map((e) => e['interest_id'].toString()).toSet();
        ...
        final int finalMatchPercentage = (gowersScore * 100).round().clamp(0, 100);
